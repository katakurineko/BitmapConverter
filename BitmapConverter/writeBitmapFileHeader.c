...
void writeBitmapFileHeader(FILE* pFile,bitmapInfoHeader* pBIH) {

	/*ファイル形式の書き込み*/
	fwrite("BM", strlen("BM"), 1, pFile);
	...
	fwrite(&bfSize, sizeof(bfSize), 1, pFile);

	/*予約領域の書き込み*/
	signed short bfReserved1 = 0;
	fwrite(&bfReserved1, sizeof(bfReserved1), 1, pFile);
	signed short bfReserved2 = 0;
	fwrite(&bfReserved2, sizeof(bfReserved2), 1, pFile);

	/*ファイル先頭から画像データまでのオフセットの書き込み*/
	unsigned long bfOffBits = BF_OFF_BITS_VALUE;
	fwrite(&bfOffBits, sizeof(bfOffBits), 1, pFile);
}